...
class Comment:
    ...
    def add_comment(cls, user_id: int, text: str,
        ...
        #print(f"[DEBUG add_comment] START - user_id={user_id}, text={text}, media_id={media_id}, parent_comment_id={parent_comment_id}")
    ...
    def update_comment(cls, user_id: int, comment_id: int, new_text: str) -> bool:
        ...
        try:
            can_manage = User.can_manage_content(user_id, owner_id)
            is_adm = User.is_admin(user_id)
        ...
        if not User.can_manage_content(user_id, owner_id) and user_id != media_owner_id:
